...
export default class LevelEditorScene extends Phaser.Scene {
  ...
  updateTerrainDisplay() {
    ...
    this.levelData.slopeZones.forEach(zone => {
      ...
      this.terrainGraphics.fillStyle(color, 0.2);
  ...
  addSlopeZone() {
    this.levelData.slopeZones.push({
      fromX: 200,
      toX: 400,
      accelMult: 1.2,
      brakeMult: 0.8,
  ...
  updateZoneList() {
    ...
    this.levelData.slopeZones.forEach((zone, index) => {
      ...
      // Zone info
      const zoneType = zone.accelMult > 1 ? 'Downhill' : 'Uphill';
      const text = this.add.text(0, 0, `${index + 1}. ${zoneType} (${zone.fromX}-${zone.toX})`, {
        fontSize: '11px',
        color: zone.accelMult > 1 ? '#cc8866' : '#6688cc'
      });
      row.add(text);

      // Delete button
      const delBtn = this.add.text(180, 0, '[X]', {
        fontSize: '11px',
        color: '#ff6666'
      }).setInteractive({ useHandCursor: true });
    ...
    if (this.levelData.slopeZones.length === 0) {
      const emptyText = this.add.text(0, 0, 'No zones (flat terrain)', {
        fontSize: '11px',
        color: '#666688'
      });
  ...
  createActionButtons() {
    ...
    // Test Play button
    this.createButton(width - 150, height - 90, 140, 'Test Play', 0x44aa44, () => {
      this.testPlayLevel();
    });

    // Save button
    this.createButton(width - 150, height - 45, 140, 'Save & Publish', 0x4466cc, () => {
      this.showSaveDialog();
    });
  }
  ...
  showSaveDialog() {
    ...
    // Title
    const title = this.add.text(width / 2, height / 2 - 90, 'Save Level', {
      fontSize: '24px',
      fontFamily: 'Arial Black',
      color: '#ffffff'
    }).setOrigin(0.5);
    ...
    // Level name label and input hint
    const nameLabel = this.add.text(width / 2 - 150, height / 2 - 50, 'Level Name:', {
      fontSize: '14px',
      color: '#aaaacc'
    });
    this.saveDialog.add(nameLabel);

    // Name input (using Phaser text that can be clicked to edit)
    this.nameInputBg = this.add.rectangle(width / 2, height / 2 - 25, 300, 30, 0x222244);
    this.nameInputBg.setStrokeStyle(1, 0x444466);
    this.saveDialog.add(this.nameInputBg);

    this.nameInputText = this.add.text(width / 2, height / 2 - 25, this.levelData.name, {
      fontSize: '16px',
      color: '#ffffff'
    }).setOrigin(0.5);
    this.saveDialog.add(this.nameInputText);

    // Author name
    const authorLabel = this.add.text(width / 2 - 150, height / 2 + 10, 'Your Name (optional):', {
      fontSize: '14px',
      color: '#aaaacc'
    });
    this.saveDialog.add(authorLabel);

    this.authorInputBg = this.add.rectangle(width / 2, height / 2 + 35, 300, 30, 0x222244);
    this.authorInputBg.setStrokeStyle(1, 0x444466);
    this.saveDialog.add(this.authorInputBg);

    this.authorInputText = this.add.text(width / 2, height / 2 + 35, 'Anonymous', {
      fontSize: '16px',
      color: '#888888'
    }).setOrigin(0.5);
    this.saveDialog.add(this.authorInputText);

    // Create DOM inputs for actual text entry
    this.createDOMInputs();

    // Save button
    const saveBtn = this.add.text(width / 2 - 60, height / 2 + 85, '[ Save ]', {
      fontSize: '20px',
      color: '#88ff88'
    ...
    // Cancel button
    const cancelBtn = this.add.text(width / 2 + 60, height / 2 + 85, '[ Cancel ]', {
      fontSize: '20px',
      color: '#aaaaaa'
    }).setOrigin(0.5).setInteractive({ useHandCursor: true });
  ...
  createDOMInputs() {
    ...
    this.nameInput.style.cssText = `
      position: absolute;
      left: ${rect.left + (480 - 150) * scaleX}px;
      top: ${rect.top + (270 - 25 - 15) * scaleY}px;
      width: ${300 * scaleX}px;
      height: ${30 * scaleY}px;
    ...
    this.authorInput.style.cssText = `
      position: absolute;
      left: ${rect.left + (480 - 150) * scaleX}px;
      top: ${rect.top + (270 + 35 - 15) * scaleY}px;
      width: ${300 * scaleX}px;
      height: ${30 * scaleY}px;
      font-size: ${16 * Math.min(scaleX, scaleY)}px;
      background: #222244;
